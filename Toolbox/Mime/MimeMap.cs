...
namespace aemarcoCommons.Toolbox.Mime
{
    public class MimeMap
    {
        ...
        /// </summary>
        /// <param name="ext">The filename or extension.</param>
        ...
        public static bool TryGetMimeType(string ext, out string mimeType)
        {
            ...
            var indexQuestionMark = ext.IndexOf(QuestionMark, StringComparison.Ordinal);
            if (indexQuestionMark != -1)
            {
                ext = ext.Remove(indexQuestionMark);
            }

            if (!ext.StartsWith(Dot))
            {
                var index = ext.LastIndexOf(Dot, StringComparison.Ordinal);
                ext = index == -1
                    ? $"{Dot}{ext}"
                    : $"{Dot}{ext.Substring(index + 1)}";
            }

            return MappingsExtToMime.Value.TryGetValue(ext, out mimeType);
        }
        ...
        public static string GetMimeType(string fileNameOrExtension) =>
            TryGetMimeType(fileNameOrExtension, out var result)
                ? result
                : DefaultMimeType;
        ...
        private static IDictionary<string, string> BuildMappingsMimeToExt()
        {
            ...
            MappingsExtToMime.Value
                .ToList()
                .ForEach(x =>
                {
                    if (!mappings.ContainsKey(x.Value))
                        mappings.Add(x.Value, x.Key);
                });
            return mappings;
        }
